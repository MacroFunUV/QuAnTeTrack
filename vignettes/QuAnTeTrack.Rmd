...
This section will guide you through installing and loading the package.
...
- **tpsUtil**: For compiling and converting `.TPS` files.  
- **tpsDig**: For digitizing footprint coordinates from trackways.  

The digitization process should ensure that the footprints are consistently recorded across all samples, with attention to proper scaling. This process is essential for converting raw images into structured data for further analysis.
...
- Reads `.TPS` files containing digitized footprints.
...
To assess whether tracks exhibit distinct movement patterns, the following statistical tests are applied:
...
- **Testing Direction (`test_direction()`)**:  
  ...
  - Performs pairwise comparisons if necessary and generates statistical measures of similarity or divergence.
...
These models can be **informed by geological data** (e.g., sedimentology, topography) to test the influence of environmental constraints on movement. For example, natural barriers or features inferred from geological evidence may restrict the range of simulated paths.

**Visualization of Simulations (`plot_sim()`)**:  
...
To compare simulated tracks against real data, various similarity metrics and intersection tests are applied:
...
The functions allow users to generate simulated datasets to compare against real tracks and assess statistical significance through permutation tests.
...
- **`data`**: A 'track' R object containing two elements: `Trajectories` (interpolated tracks) and `Footprints` (original footprint data).  
- **`plot`**: Specifies the type of plot to generate. Options are `"FootprintsTracks"` (default), `"Tracks"`, or `"Footprints"`.  
- **`colours`**: A vector of colors for different tracks. If `NULL`, defaults to black.  
- **`cex.f`**: Controls the size of footprint points. Default is `2.5`.  
- **`shape.f`**: A vector specifying shapes for footprints in different tracks. Default is solid circle (`19`).  
- **`alpha.f`**: Sets the transparency of footprint points. Default is `0.5`.  
- **`cex.t`**: Controls the size of track lines. Default is `0.5`.  
- **`alpha.t`**: Sets the transparency of track lines. Default is `1`.  
- **`plot.labels`**: Logical argument indicating whether to display labels for each track. Default is `FALSE`.  
- **`labels`**: A vector of labels for each track. If `NULL`, labels are automatically generated.  
- **`box.p`**: Controls the padding around label boxes if `plot.labels = TRUE`.  
- **`cex.l`**: Controls the size of labels. Default is `3.88`.  
- **`alpha.l`**: Sets the transparency of the labels. Default is `0.5`.  
...
Additionally, the sinuosity calculation is based on the following formula:
\[
