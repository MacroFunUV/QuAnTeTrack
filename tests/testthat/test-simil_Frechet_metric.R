test_that("simil_Frechet_metric returns a valid similarity object for Directed model with PaluxyRiver dataset", {
  ...
  expect_true(all(c("Frechet_distance_metric", "Frechet_distance_metric_p_values",
                    "Frechet_metric_p_values_combined", "Frechet_distance_metric_simulations") %in% names(result)))
})
...
test_that("simil_Frechet_metric handles invalid inputs correctly", {
  ...
  expect_error(simil_Frechet_metric(PaluxyRiver, test = TRUE, sim = suppressWarnings(simulate_track(PaluxyRiver, nsim = 10)), superposition = "Invalid"),
