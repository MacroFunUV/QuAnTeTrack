...
test_that("plot_sim returns a ggplot object for Directed model simulation with MountTom dataset", {
  ...
  plot_result <- plot_sim(sbMountTom, simulated_tracks_mt_directed,
                          colours_sim = c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00",
...
test_that("plot_sim returns a ggplot object for Constrained model simulation with MountTom dataset", {
  ...
  plot_result <- plot_sim(sbMountTom, simulated_tracks_mt_constrained,
                          colours_sim = c("#E41A1C", "#377EB8", "#4DAF4A", "#FF7F00", "#F781BF", "#A65628",
...
test_that("plot_sim returns a ggplot object for Unconstrained model simulation with MountTom dataset", {
  ...
  plot_result <- plot_sim(sbMountTom, simulated_tracks_mt_unconstrained,
                          colours_sim = c("#6BAED6", "#FF7F00", "#1F77B4", "#D62728", "#2CA02C", "#9467BD",
                                          "#8C564B", "#E377C2", "#7F7F7F", "#BCBD22", "#17BECF"),
                          alpha_sim = 0.2, lwd_sim = 0.5, colours_act = c("#6BAED6", "#FF7F00", "#1F77B4",
                                                                          "#D62728", "#2CA02C", "#9467BD", "#8C564B", "#E377C2",
                                                                          "#7F7F7F", "#BCBD22", "#17BECF"), alpha_act = 1, lwd_act = 2)
...
test_that("plot_sim handles invalid inputs correctly", {
  ...
  expect_error(plot_sim(PaluxyRiver, suppressWarnings(simulate_track(PaluxyRiver)), colours_sim = c("#E69F00")),
  ...
  expect_error(plot_sim(PaluxyRiver, suppressWarnings(simulate_track(PaluxyRiver)), colours_act = c("#E69F00")),
  ...
  expect_error(plot_sim(PaluxyRiver, suppressWarnings(simulate_track(PaluxyRiver)), alpha_sim = -0.5),
               "Alpha value for simulated trajectories 'alpha_sim' should be between 0 and 1.")
  expect_error(plot_sim(PaluxyRiver, suppressWarnings(simulate_track(PaluxyRiver)), alpha_act = 1.5),
               "Alpha value for actual trajectories 'alpha_act' should be between 0 and 1.")
  expect_error(plot_sim(PaluxyRiver, suppressWarnings(simulate_track(PaluxyRiver)), lwd_sim = 0),
               "Line width value for simulated trajectories 'lwd_sim' should be a positive number.")
  expect_error(plot_sim(PaluxyRiver, suppressWarnings(simulate_track(PaluxyRiver)), lwd_act = -1),
