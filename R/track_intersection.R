...
#' - \code{"Lower"}: tests whether the observed intersections are significantly fewer
...
#' - \code{"Higher"}: tests whether the observed intersections are significantly greater
#' than those generated by simulations. This scenario corresponds to hypotheses
...
#'
#' The interpretation of the \strong{combined global *p*-value} returned by the function
...
#'
#' \item{Intersection_metric_p_values}{(If \code{test = TRUE}) A numeric matrix of raw pairwise *p*-values,
...
#' the combined *p*-value across all intersections (based on the global statistic: the sum of pairwise intersections).
...
track_intersection <- function(data, test = NULL, H1 = NULL, sim = NULL, origin.permutation = NULL, custom.coord = NULL) {
  ...
  if (test == TRUE) {
    ...
    Intersectsim_pval_BH <- as.data.frame(matrix(vals_adj,
                                                 nrow = nrow(tmp),
                                                 ncol = ncol(tmp),
                                                 byrow = FALSE))
