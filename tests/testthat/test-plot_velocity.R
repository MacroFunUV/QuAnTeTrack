...
test_that("plot_velocity handles invalid inputs correctly", {
  ...
  expect_error(plot_velocity(MountTom, velocity_from_stride_track(MountTom, H = H_mounttom), param = "Invalid value for 'param'. Choose 'V' for velocity, 'RSL' for relative stride length, or NULL."))
  expect_error(plot_velocity(MountTom, velocity_from_stride_track(MountTom, H = H_mounttom), colours = 5), "'colours' must be a character vector of color specifications.")
  expect_error(plot_velocity(MountTom, velocity_from_stride_track(MountTom, H = H_mounttom), colours = c(2, 4)), "'colours' must be a character vector of color specifications.")
})
