...
```{r, eval=FALSE}
sim_directed_paluxy <- simulate_track(PaluxyRiver, nsim = 1000, model = "Directed")
...
```{r, eval=FALSE}
sim_constrained_paluxy <- simulate_track(PaluxyRiver, nsim = 1000, model = "Constrained")
...
```{r, eval=FALSE}
sim_unconstrained_mount <- simulate_track(sbMountTom, nsim = 1000, model = "Unconstrained")
...
```{r, eval=FALSE}
sim_directed_mount <- simulate_track(sbMountTom, nsim = 1000, model = "Directed")
...
```{r, eval=FALSE}
sim_constrained_mount <- simulate_track(sbMountTom, nsim = 1000, model = "Constrained")
...
The **`simil_DTW_metric()`** and **`simil_Frechet_metric()`** functions provide robust tools for comparing **movement trajectories** by quantifying their similarity through distinct mathematical approaches. Both functions operate on a **`track` object**, allowing users to evaluate whether observed patterns deviate from random expectations by comparing real tracks to simulated ones. 
...
Statistical testing is supported when **`test = TRUE`**, requiring a **`track simulation` object** to be provided through the **`sim`** argument. This allows users to compare similarity metrics between real tracks and simulated trajectories, determining whether observed similarities are significantly greater than those expected under random conditions. The functions provide two types of ***p*-values**: **pairwise *p*-values**, which represent the proportion of simulated distances smaller than the observed distance for each trajectory pair, and **combined *p*-values**, which reflect the overall proportion of simulations where all observed distances are smaller than the simulated ones. This statistical framework offers a comprehensive approach for evaluating similarity metrics and testing hypotheses about track similarity.

The **`simil_DTW_metric()`** and **`simil_Frechet_metric()`** functions return a **`track similarity` object**, which contains valuable information about the similarity between trajectories. This object includes a matrix called **`DTW_distance_metric`** (for the DTW metric) or its equivalent for the Fréchet metric, which stores the pairwise distances calculated between all trajectories. If the **`test`** argument is set to **`TRUE`**, additional components are included in the output. Specifically, a matrix of **`DTW_distance_metric_p_values`** is provided, containing the p-values for the pairwise distances obtained by comparing the observed tracks to those generated through simulations. The **`DTW_metric_p_values_combined`** element reports the overall p-value, summarizing the significance of the similarity between all tracks when considering the simulated datasets. Moreover, the output includes **`DTW_distance_metric_simulations`**, a list of matrices with the similarity metrics calculated from each simulated dataset. This comprehensive output structure allows users to thoroughly evaluate whether the observed similarity between trajectories is greater than expected under random conditions, providing robust statistical support for their analyses.
...
The input to this function is a **`track` object**. If statistical testing is desired, **`test`** argument must be set **`TRUE`** and a **`track simulation` object** provided via the **`sim`** argument.
...
Detecting intersections between **MountTom** tracks (after subsetting) and simulated tracks generated using the **Constrained model**, with **convex hull permutation** of starting points (**`origin.permutation = "Conv.Hull"`**), and testing for **increased intersections** (**H1 = "Higher"**).  
