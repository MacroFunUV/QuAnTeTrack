...
test_that("plot_velocity returns a ggplot object for sequence velocity plots (MountTom dataset)", {
  H_mounttom <- c(
    1.380, 1.404, 1.320, 1.736, 1.364, 1.432, 1.508, 1.768, 1.600, 1.848,
    1.532, 1.532, 0.760, 1.532, 1.688, 1.620, 0.636, 1.784, 1.676, 1.872,
    1.648, 1.760, 1.612
  ...
  V_mounttom <- velocity_track(MountTom, H = H_mounttom)
  plot_result <- plot_velocity(MountTom, V_mounttom, type = "sequence", param = "V")

  expect_s3_class(plot_result, "ggplot")
})

test_that("plot_velocity returns a ggplot object for sequence relative stride length plots (MountTom dataset)", {
  H_mounttom <- c(
    1.380, 1.404, 1.320, 1.736, 1.364, 1.432, 1.508, 1.768, 1.600, 1.848,
    1.532, 1.532, 0.760, 1.532, 1.688, 1.620, 0.636, 1.784, 1.676, 1.872,
    1.648, 1.760, 1.612
  ...
  V_mounttom <- velocity_track(MountTom, H = H_mounttom)
  plot_result <- plot_velocity(MountTom, V_mounttom, type = "sequence", param = "RSL")
...
test_that("plot_velocity handles invalid inputs correctly", {
  ...
  expect_error(plot_velocity(MountTom, velocity_track(MountTom, H = H_mounttom), type = "invalid"),
               "Invalid value for 'type'. Choose 'tracksite', 'sequence', or NULL.")
  expect_error(plot_velocity(MountTom, velocity_track(MountTom, H = H_mounttom), param = "invalid"),
               "Invalid value for 'param'. Choose 'V' for velocity, 'RSL' for relative stride length, or NULL.")
  expect_error(plot_velocity(MountTom, velocity_track(MountTom, H = H_mounttom), colours = 5),
               "'colours' must be a character vector of color specifications.")
  expect_error(plot_velocity(MountTom, velocity_track(MountTom, H = H_mounttom), colours = c(2, 4)),
               "'colours' must be a character vector of color specifications.")
})
