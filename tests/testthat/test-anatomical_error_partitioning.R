test_that("anatomical_error_partitioning basic structure and contents", {
  ...
  vars <- c("PathLen","BeelineLen","Straightness","Sinuosity","StLength")

  trk <- make_data()
  res <- anatomical_error_partitioning(trk,
                                       error_radius = 0.05,
                                       variables = vars,
                                       gauge_size = 1,     # needed because Gauge is in the default set, but not here
                                       n_sim = 20,
  ...
  expect_true(all(res$summary$component %in% c("trackway","anatomical","Residual")))
  ...
  expect_true(is.factor(res$analysis_table$trackway))
...
test_that("anatomical_error_partitioning respects distribution and seed", {
  ...
  trk <- make_data()
  a <- anatomical_error_partitioning(trk, 0.05, variables = vars, gauge_size = 1,
                                     n_sim = 25, distribution = "gaussian", seed = 101)
  b <- anatomical_error_partitioning(trk, 0.05, variables = vars, gauge_size = 1,
                                     n_sim = 25, distribution = "gaussian", seed = 101)
  c <- anatomical_error_partitioning(trk, 0.05, variables = vars, gauge_size = 1,
                                     n_sim = 25, distribution = "uniform",  seed = 101)
...
test_that("error_radius handling: scalar recycling and per-trackway vector", {
  skip_if_not_installed("trajr")
  ...
  trk <- make_data()
  r1 <- anatomical_error_partitioning(trk, 0.05, variables = vars, gauge_size = 1,
                                      n_sim = 15, distribution = "uniform", seed = 7)
  r2 <- anatomical_error_partitioning(trk, c(0.05, 0.05), variables = vars, gauge_size = 1,
                                      n_sim = 15, distribution = "uniform", seed = 7)
  expect_equal(r1$summary$variance, r2$summary$variance, tolerance = 1e-12)

  r3 <- anatomical_error_partitioning(trk, c(0.02, 0.08), variables = vars, gauge_size = 1,
                                      n_sim = 15, distribution = "uniform", seed = 7)
...
test_that("edge regimes: radius=0 gives zero anatomical; identical trackways give near-zero biological", {
  skip_if_not_installed("trajr")
  ...
  trk <- make_data()
  res0 <- anatomical_error_partitioning(trk, 0,
                                        variables = c("PathLen","BeelineLen"),
                                        gauge_size = 1,
  ...
  resI <- anatomical_error_partitioning(trk2, 0.05,
                                        variables = c("PathLen","BeelineLen"),
                                        gauge_size = 1,
                                        n_sim = 30, distribution = "gaussian", seed = 2)
  bioI  <- subset(resI$summary, component == "trackway")
...
test_that("percent sums ~100 and R2_c in [0,1]", {
  ...
  vars <- c("PathLen","BeelineLen","Straightness")
  trk <- make_data()
  res <- anatomical_error_partitioning(trk, 0.05, variables = vars, gauge_size = 1,
...
test_that("trackway_names preserved or sensibly defaulted", {
  skip_if_not_installed("trajr")
  ...
  trk <- make_data()
  res <- anatomical_error_partitioning(trk, 0.05, variables = "PathLen", gauge_size = 1,
                                       n_sim = 10, distribution = "uniform", seed = 10)
  expect_equal(res$trackway_names, c("A","B"))

  trk2 <- trk
  names(trk2$Trajectories) <- c("", "")
  res2 <- anatomical_error_partitioning(trk2, 0.05, variables = "PathLen", gauge_size = 1,
                                        n_sim = 10, distribution = "uniform", seed = 10)
  expect_equal(res2$trackway_names, c("Trackway_01","Trackway_02"))
})
...
test_that("input validation errors and warnings follow current behavior", {
  ...
  expect_warning(
    anatomical_error_partitioning(trk, 0.05, variables = c("PathLen","NopeVar"),
                                  gauge_size = 1,
  ...
  # Gauge requested -> must error if gauge_size missing/invalid
  expect_error(
    anatomical_error_partitioning(trk, 0.05, variables = "Gauge",
                                  n_sim = 5, distribution = "uniform", seed = 1)
  )
  expect_error(
    anatomical_error_partitioning(trk, 0.05, variables = "Gauge", gauge_size = NA,
                                  n_sim = 5, distribution = "uniform", seed = 1)
  )
  expect_error(
    anatomical_error_partitioning(trk, 0.05, variables = "Gauge", gauge_size = -1,
                                  n_sim = 5, distribution = "uniform", seed = 1)
  )

  # n_sim invalid -> errors (character first warns, then errors)
  expect_error(anatomical_error_partitioning(trk, 0.05, gauge_size = 1, n_sim = 0))
  expect_error(anatomical_error_partitioning(trk, 0.05, gauge_size = 1, n_sim = -2))
  expect_warning(expect_error(anatomical_error_partitioning(trk, 0.05, gauge_size = 1, n_sim = "ten")))

  # distribution invalid -> error (match.arg)
  expect_error(anatomical_error_partitioning(trk, 0.05, gauge_size = 1, distribution = "weird"))

  # seed non-numeric -> error from set.seed(); suppress base coercion warning
  expect_error(suppressWarnings(anatomical_error_partitioning(trk, 0.05, gauge_size = 1, n_sim = 5, seed = "seed")))
})
